...
static bool parse_entity(
        const char *current, char **to, const char **from, const char* string_end)
{
    const char *end = memchr(current, ';', string_end - current);
    if(!end) return 0;
...
size_t decode_html_entities_utf8(char *dest, const char *src, const char* src_end)
{
    ...
    for(const char *current; (current = memchr(from, '&', src_end - from));)
    {
        memmove(to, from, (size_t)(current - from));
        to += current - from;

        if(parse_entity(current, &to, &from, src_end))
            continue;
