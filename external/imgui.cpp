...
/*
 ...
 - Read the "API BREAKING CHANGES" section (below). This is where we list occasional API breaking changes.
   If a function/type has been renamed / or marked obsolete, try to fix the title in your code before it is permanently removed from the public API.
   If you have a problem with a missing function/symbols, search for its title in the code, there will likely be a comment about it.
 ...
 - tip: you can call Begin() multiple times with the same title during the same frame, it will keep appending to the same window.
...
// - Begin/End can be called multiple times during the frame with the same window title to append content.
// - The window title is used as a unique identifier to preserve window information across frames (and save rudimentary information to the .ini file).
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL);                        // Window title required
    IM_ASSERT(g.Initialized);                       // Forgot to call ImGui::NewFrame()
...
{
    // Hexadecimal input provided as a convenience but the flag title is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users title their columns set the same as another widget.
